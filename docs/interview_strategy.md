...
2. 不能说已经生产可用。
3. 要主动补充当前已加入 adversarial query、error analysis 和 release gate，后续还会继续扩展多业务线、多时间范围和真实用户反馈样本。
...
1. `docs/demo_walkthrough.md`：按 30 秒、3 分钟和 8 分钟三种节奏串起三个项目。
2. 根 README 增加导览入口。
...
### 已完成：扩展 Project 02 的评测可信度

已新增：

1. `adversarial_queries.csv`：诱导模型生成不存在字段、越权查询、危险 SQL、敏感字段和因果过度推断。
2. `error_analysis.md`：列出 V1/V2/V3 典型失败样本。
3. `release_gate.md`：明确 prompt 版本进入内测或灰度前需要满足的门槛。

待补充：

1. 把 adversarial queries 接入可执行 evaluator。
2. 更多时间范围和多指标问题：比如最近 30 天、环比、同比、多地区对比。

### 已完成：让 Project 03 更像真实评审材料
...
下一步不要继续大改 Project 01 和 Project 02 的主逻辑。建议先完成三件小事：

1. 打开 GitHub 检查根 README、Project 02 扩展材料和 Project 03 新增评审材料。
2. 手动补 GitHub About 的 description 和 topics。
